...
async def hideMessages(message, msg, msgID, msgArgs):
    ...
    maxLogMsgs = 50
    if len(msgArgs) >= 2:
      taggedUser, maxLogMsgs = await checkTaggedUser(msgArgs[1], msgID, maxLogMsgs)
    else:
      taggedUser = msgID
    if taggedUser == "Invalid":
        await message.channel.send("Correct usage: `!hide` or `!hide me` or `!hide @user`")
        return  
...
async def deleteMsg(message, msg, msgID, msgArgs):
    ...
    if len(msgArgs) >= 3:
        taggedUser, maxLogMsgs = await checkTaggedUser(msgArgs[2], msgID, maxLogMsgs)
        if taggedUser == "Invalid":
          await message.channel.send("Please end the !del command with 'me' or by tagging a user")
          return  
        await deleteHelper(message, numMsgs, taggedUser, maxLogMsgs)
    else:
